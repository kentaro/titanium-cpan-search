...
{
	if (self = [super init]) {
		_cpan_searchRoot = [rootController retain];
		proxy = split_;
...
-(void)dealloc
{
	RELEASE_TO_NIL(_cpan_searchRoot);
...
-(void)loadView
{
	[[NSNotificationCenter defaultCenter] removeObserver:_cpan_searchRoot name:UIDeviceOrientationDidChangeNotification object:nil];
	[super loadView];
}
...
{
	[_cpan_searchRoot refreshOrientationModesIfNeeded:oldCurrentWindow];
}
