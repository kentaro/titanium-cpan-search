...
@implementation cpan_searchObject
...
-(void)injectPatches
{
	// called to inject any cpan_search patches in JS before a context is loaded... nice for 
...
{
	// create cpan_search global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_cpan_search = [[cpan_searchObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_cpan_search];
	
	NSString *_cpan_searchNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_cpan_searchNS UTF8String]);
	...
	if (preload!=nil)
	{
		KrollObject *ti = (KrollObject*)[_cpan_search valueForKey:@"UI"];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.cpan_search.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
